...
class SequencePredictor(Model):
    ...
    def add_training_op(self, loss):
        ...
        gradient_variable_pairs = optimizer.compute_gradients(loss)
        gradients = [g for g, v in gradient_variable_pairs]
        variables = [v for g, v in gradient_variable_pairs]
        if self.config.clip_gradients:
            gradients_clipped, _ = tf.clip_by_global_norm(gradients, self.config.max_grad_norm)
            train_op = optimizer.apply_gradients(zip(gradients_clipped, variables))
            self.grad_norm = tf.global_norm(gradients_clipped)
        else:
            train_op = optimizer.apply_gradients(gradient_variable_pairs)
